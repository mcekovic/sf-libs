...
public class LocalRepositoryEntityCacheDisabledTest {
	...
	public void setUp() {
		...
		repository = new TransactionalRepository<>(new LocalDomainContext(), dao, false, false, false);
		repository.init();
	...
	public void newEntityWithIdIsCreatedAndGet() {
		...
		when(dao.fetch(2L)).thenReturn(entity.deepClone());
	...
	public void exitingEntityIsSavedAndGet() {
		...
		when(dao.fetch(3L)).thenReturn(entity.deepClone());

		SimpleTestEntity entity2 = repository.get(entity.getId());

		verify(dao, times(2)).fetch(3L);
		verifyNoMoreInteractions(dao);
		assertThat(entity2.getName()).isEqualTo(entity.getName());
		assertThat(entity2).isNotSameAs(entity);
	}
